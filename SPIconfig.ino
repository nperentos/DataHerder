...
void SPI_init() {
  ...
  Serial.println(F("Begin and configuring SPI bus now..."));
  SPI.begin();
  SPI.setSCK(SCKpin);
  SPI.setMOSI(MOSIpin);
  SPI.setMISO(MISOpin);
...
void SPI_verify() {
  ...
  //SPI.beginTransaction(SPISettings(SPI_TRANSFER_RATE_DIV, MSBFIRST, SPI_MODE0));
  ...
  int thisCSpin;
  for (unsigned int k = 0; k < 2 ; k++) { //loop twice through, once for A, once for B, set chip select accordingly

    // set chip select pin to 10 or 9

    if (k == 0) {
      thisCSpin = CSpinA;  //for amp A
      Serial.print(F("Amp A verifying SPI link now. CS pin is:"));
      Serial.println(thisCSpin);
      
    }
    else {
      thisCSpin = CSpinB;  // for ampB
      Serial.print(F("Amp B verifying SPI link now. CS pin is:"));
      Serial.println(thisCSpin);
    }
    ...
    Serial.println(F("Reading and printing contents of registers 40-44"));
    Serial.println(F("Check link is verified.  Should spell out INTAN below:"));
    for (unsigned int n = 0; n < (sizeof(CompanyDesignation) / sizeof(CompanyDesignation[0])); n++) {
      Serial.write(CompanyDesignation[n]); //prints out in ASCII
//      Serial.print(CompanyDesignation[n]);
    }
        
    Serial.println(); //prints out in ASCII
...
uint16_t SPI_getAmplInfo() { // returns number of amplifiers on chip
  ...
  for (int k = 0; k < 2 ; k++) { //loop twice through, once for A, once for B, set chip select accordingly
    ...
    if (k == 0) {
      thisCSpin = CSpinA;  //for amp A
//      Serial.println("amp A get info now.");
    }
    else {
      thisCSpin = CSpinB;  // for ampB
//      Serial.println("amp B get info now.");
    }
...
//      Serial.print("Chip ID: RHD2132");//value of 1 encodes for RHD2132 chip
//      Serial.println("");//print new line
    }
    else { //could enter print statements for other chips here, but we won't bother now
 //     Serial.print("Amplifier Reg 63: ");//value of 1 encodes for RHD2132 chip
 //     Serial.println(AmplifierInfo[3]);//value of 1 encodes for RHD2132 chip
    }

//    Serial.print("Number of amplifiers: ");
//    Serial.println(AmplifierInfo[2]);
    if (AmplifierInfo[1] == 1) { //unipolar = common reference design
//      Serial.println("Unipolar configuration = common reference");//value of 1 encodes for unipolar config characteristic of RHD2132
    }

//    Serial.print("Die revision: ");
//    Serial.println(AmplifierInfo[0]);
  }

  SPI.endTransaction();
//  Serial.write(TERM_CHAR); //write termination character, used to sync with LabView
//  Serial.flush(); // wait for all data in Tx buffer to transmit

  Serial.print(F("Number of INTAN chips detected = "));
  Serial.println((numAmps+1)/32);
